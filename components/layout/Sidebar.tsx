...
function getIcon(label: string) {
    ...
    if (l.includes('dashboard') || l.includes('overview') || l.includes('hub') || l.includes('platform')) return LayoutDashboard;
    if (l.includes('home')) return Home;
    if (l.includes('schedule') || l.includes('timetable') || l.includes('calendar') || l.includes('academic')) return Calendar;
    if (l.includes('exam') || l.includes('score') || l.includes('result') || l.includes('marks') || l.includes('grade')) return Award;
    if (l.includes('attendance')) return BarChart2;
    if (l.includes('analytics') || l.includes('deep') || l.includes('pulse')) return BarChart3;
    if (l.includes('message') || l.includes('chat') || l.includes('forum') || l.includes('peer')) return MessageSquare;
    if (l.includes('live cl') || l.includes('video')) return Video;
    ...
    if (l.includes('user') || l.includes('student') || l.includes('directory') || l.includes('faculty') || l.includes('admin') || l.includes('registry')) return Users;
    if (l.includes('department') || l.includes('institutional')) return Building2;
    if (l.includes('circular') || l.includes('archive') || l.includes('announcement') || l.includes('notification') || l.includes('global')) return Bell;
    if (l.includes('infra') || l.includes('room') || l.includes('asset') || l.includes('batch') || l.includes('segment')) return Layers;
    if (l.includes('performance') || l.includes('report') || l.includes('node')) return Activity;
    if (l.includes('role') || l.includes('access') || l.includes('control')) return Shield;
    if (l.includes('ai') || l.includes('generator') || l.includes('smart') || l.includes('buddy')) return Zap;
    if (l.includes('note')) return BookMarked;
    if (l.includes('roadmap') || l.includes('path')) return Map;
    if (l.includes('test') || l.includes('quiz') || l.includes('assignment')) return ClipboardList;
    if (l.includes('alumni') || l.includes('career') || l.includes('skill') || l.includes('assessment')) return Star;
    if (l.includes('backup')) return ShieldCheck;
    if (l.includes('log') || l.includes('audit') || l.includes('record') || l.includes('help') || l.includes('support')) return FileText;
    if (l.includes('risk') || l.includes('grievance') || l.includes('intervention')) return Activity;
    ...
    return (
        <aside className="w-64 h-screen fixed left-0 top-0 z-50 flex flex-col shadow-2xl overflow-hidden bg-[#0f172a] text-white border-r border-white/5">
            ...
            <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto scrollbar-thin">
                <p className="px-3 pt-2 pb-2 text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">Navigation</p>
                {links.map((link) => {
            ...
            <div className="p-3 border-t border-white/5 space-y-1 shrink-0">
                <Link href="/dashboard/profile" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[12px] font-semibold text-slate-400 hover:bg-white/5 hover:text-white transition-all">
                    <UserCircle size={15} className="text-slate-500" /> Settings
                </Link>
                <button
                    onClick={() => signOut({ callbackUrl: '/portals' })}
                    className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[12px] font-bold text-rose-400 hover:bg-rose-500/10 transition-all text-left"
                >
...
function LmsSidebarInner({ role }: { role: Role }) {
    ...
    return (
        <aside className="w-64 h-screen fixed left-0 top-0 z-50 flex flex-col shadow-2xl overflow-hidden bg-[#052e16] text-white border-r border-emerald-900/60">
            {/* Brand Header */}
            <div className="px-6 py-6 border-b border-emerald-900/60 shrink-0">
                <div className="flex items-center gap-3 mb-4">
            ...
            {/* Nav */}
            <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto scrollbar-thin">
                <p className="px-3 pt-2 pb-2 text-[9px] font-black uppercase tracking-[0.2em] text-emerald-800">Learning Modules</p>
                {links.map((link) => {
                    const Icon = getIcon(link.label);
                    const isActive = pathname === link.href;
                    return (
            ...
            {/* Footer */}
            <div className="p-3 border-t border-emerald-900/60 space-y-1 shrink-0">
                <Link href="/dashboard/profile" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[12px] font-semibold text-emerald-100/60 hover:bg-emerald-900/60 hover:text-white transition-all">
                    <UserCircle size={15} className="text-emerald-700" /> Settings
                </Link>
                <button
                    onClick={() => signOut({ callbackUrl: '/portals' })}
                    className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[12px] font-bold text-rose-400 hover:bg-rose-500/10 transition-all text-left"
...
function ErpSidebarInner({ role }: { role: Role }) {
    ...
    return (
        <aside className="w-64 h-screen fixed left-0 top-0 z-50 flex flex-col shadow-2xl overflow-hidden bg-[#1c0a0a] text-white border-r border-rose-950/60">
            {/* Brand Header */}
            <div className="px-6 py-6 border-b border-rose-950/60 shrink-0">
                <div className="flex items-center gap-3 mb-4">
            ...
            {/* Nav */}
            <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto scrollbar-thin">
                <p className="px-3 pt-2 pb-2 text-[9px] font-black uppercase tracking-[0.2em] text-rose-950">Student Space</p>
                {links.map((link) => {
                    const Icon = getIcon(link.label);
                    const isActive = pathname === link.href;
                    return (
            ...
            {/* Footer */}
            <div className="p-3 border-t border-rose-950/60 space-y-1 shrink-0">
                <Link href="/dashboard/profile" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[12px] font-semibold text-rose-100/60 hover:bg-rose-950/60 hover:text-white transition-all">
                    <UserCircle size={15} className="text-rose-900" /> Settings
                </Link>
                <button
                    onClick={() => signOut({ callbackUrl: '/portals' })}
                    className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[12px] font-bold text-rose-400 hover:bg-rose-500/10 transition-all text-left"
...
// ─────────────────────────────────────────────
// Smart wrapper — reads URL query param or prop to pick sidebar
// ─────────────────────────────────────────────
